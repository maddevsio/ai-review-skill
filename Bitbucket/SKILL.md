...
```bash
python3 .claude/skills/pr-review/scripts/check-credentials.py
```

If this fails, tell the user their Bitbucket credentials are not configured and guide them through setup:
1. Run `python3 .claude/skills/pr-review/scripts/setup.py` to generate `.claude/settings.local.json`
2. Open `.claude/settings.local.json` and fill in:
   - `BITBUCKET_USER` — your Bitbucket account email
   - `BITBUCKET_TOKEN` — App Password from Bitbucket Account Settings → App passwords
...
- **Argument:** PR URL or PR number (e.g., `https://bitbucket.org/<workspace>/<repo>/pull-requests/123` or `123`)
...
2. Run `python3 .claude/skills/pr-review/scripts/get-pr.py <number>` to get branch info and changed files. Parse the JSON output to extract `headRefName`, `baseRefName`, and `files`.
...
2. Parse all blocks. For each block:
   a. Run to get code context (use `START_LINE` for multi-line comments):
      ```bash
      python3 .claude/skills/pr-review/scripts/show-context.py <number> <file> <line>
      ```
