...
Usage:
    python3 show-context.py <mr_number> <file_path> <line>
    python3 show-context.py <mr_number> <file_path> <start_line> <end_line>

Reads from /tmp/pr-diff-<mr_number>.txt (written by annotate-diff.py).
...
def main() -> None:
    if len(sys.argv) < 4:
        print(
            f"Usage: {sys.argv[0]} <mr_number> <file_path> <line> [end_line]",
            file=sys.stderr,
        )
        sys.exit(1)
    ...
    diff_path = f"/tmp/pr-diff-{mr_number}.txt"
    try:
